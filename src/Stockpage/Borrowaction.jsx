...
function Borrowaction({ onSave, Username = localStorage.getItem("fullname"), children }) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            for (const serialItem of selectedSerials) {
                // บันทึกลงใน tb_log_product เท่านั้น (ไม่มีการอัพเดต tb_borrow)
                await saveLogEntry(serialItem);
            }

            // อัพเดต stock และ tb_borrow (เรียกแยกจากการบันทึกลอก)
            for (const serialItem of selectedSerials) {
    ...
    const saveLogEntry = async (serialItem) => {
        try {
            const queryString =
                ...
                `&status=${encodeURIComponent("Borrow")}` +
    ...
    return (
        <div className="d-flex justify-content-end">
            <Button className="fw-bold" variant="warning" onClick={handleShow}>
                {children}
            </Button>

...
function QuotationCopy() {
  ...
  const calOverall = async () => {
    if (qo_totalprice != 0 && totalPriceRef.current.value == "") {
  ...
  const setOldItem = async () => {
    const updatedRows = qoProducts.map((item, index) => ({
      id: index > 0 ? index + 1 : index,
      name: `${item.pd_id}-${item.pd_name}`,
      des: `${item.pd_description}`,
      price: `${item.pd_sellprice}`,
      qty: item.item_qty,
      sumprice: item.pd_sellprice * item.item_qty,
    }));
  ...
  return (
    <>
      <div className="m-5">
        <h1 className="mb-3">Create Quotation</h1>

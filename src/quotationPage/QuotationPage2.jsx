...
function QuotationPage2() {
    ...
    do {
      ...
      try {
        const res = await fetch(
          ...
          }&db=${userInfo.name_db}&action=getwhereqoidipc&id=${fullId}`,
          {
            method: "GET",
          }
        );
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_SERVER}/Product.php?server=${
          ...
          userInfo.password_db
        }&db=${userInfo.name_db}&action=getmodelipc`,
        {
          method: "GET",
        }
      );
  ...
  const getProductDetail = async (model_id) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_SERVER}/Product.php?server=${
          userInfo.server_db
          ...
          userInfo.password_db
        }&db=${userInfo.name_db}&action=getmodelipcdetail&id=${model_id}`,
        {
          method: "GET",
        }
      );
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_SERVER}/Quotation.php?server=${
          ...
          userInfo.name_db
        }&action=insertipc&quotationId=${qoId}&reviseid=00&customerId=${
        ...
        rows.map(async (el) => {
          ...
          const res = await fetch(
            `${import.meta.env.VITE_SERVER}/Quotation.php?server=${
              ...
              userInfo.name_db
            }&action=insertitemipc&quotationId=${qoId}&reviseid=00&productId=${
              el.name
            }&item_qty=${el.qty}&item_price=${el.price}&modelType=${
              el.modelType
            }&sizeScreen=${el.sizeScreen}&cpu=${el.cpu}&mainboard=${
              el.mainboard
            }&adapter=${el.adapter}&os=${el.os}&warranty=${el.warranty}&ram=${
              el.ram
            }
            &storage=${el.storage}&rj45Qty=${el.rj45Qty}&usbQty=${
              el.usbQty
            }&hdmiVgaQty=${el.hdmiVgaQty}&wifi=${el.wifi}&detail=${
              el.detail
            }&comport=${el.comport}`,
            {
              method: "POST",
  ...
  const addRow = () => {
    const newRow = {
      ...
      warranty: "", // Warranty
      ram: "", // RAM
      storage: "", // Harddisk & Storage
      rj45Qty: "", // RJ45
      comport: "",
      usbQty: "", // USB 3.0 & USB 2.0
      hdmiVgaQty: "", // HDMI & VGA
      wifi: "", // Wifi
      detail: "", // Detail
    };
          ...
          <Typeahead
            ...
            onChange={async (selected) => {
              if (selected && selected.length > 0) {
                ...
                // await handleRowChange(split[2], rowIndex, "price"); //add sell price product
                setProductsDetail([]);
                getProductDetail(selected[0].value);
                handleRowChange(selected[0].value, rowIndex, "name");
              }
            }}
            options={products.map((product) => ({
              value: `${product.model_id}`,
              // -${product.pd_name}|${product.pd_description}|${product.pd_sellprice}`,
              label: `${product.model_id}-${product.model_name}`,
            }))}
      ...
      <td>
        ...
        <Form.Group as={Col} md="12">
          ...
          <Typeahead
            ...
            //   }
            // }}
            options={productsDetail.map((product) => ({
              value: `${product.product_cate_id}`,
              // -${product.pd_name}|${product.pd_description}|${product.pd_sellprice}`,
              label: `${product.model_id}/${product.cpu}/${product.size_screen}/${product.OS}/${product.warranty}`,
            }))}
            placeholder="Select Product"
          />
        </Form.Group> */}
        ...
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            options={productsDetail.map((product) => ({
              ...
              label: `${product.model_type}`,
            }))}
            placeholder="Select Product"
            className="flex-grow-1"
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "modelType");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "modelType");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            labelKey="label"
            options={productsDetail.map((product) => ({
              value: product.size_screen,
              label: `${product.size_screen}`,
            }))}
            placeholder="Select Product"
            className="flex-grow-1"
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "sizeScreen");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "sizeScreen");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "cpu");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "cpu");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            labelKey="label"
            options={productsDetail.map((product) => ({
              value: product.mainboard,
              label: `${product.mainboard}`,
            }))}
            placeholder="Select Product"
            className="flex-grow-1"
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "mainboard");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "mainboard");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "adapter");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "adapter");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "os");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "os");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "warranty");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "warranty");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "ram");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "ram");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          <Form.Label className="me-2 mb-0">
            <b>Harddisk & Storage :</b>
          </Form.Label>
          <Typeahead
            id="basic-typeahead-single"
            labelKey="label"
            ...
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "storage");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "storage");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            labelKey="label"
            options={productsDetail.map((product) => ({
              value: product.rj45_qty,
              label: `${product.rj45_qty}`,
            }))}
            placeholder="Select Product"
            className="flex-grow-1"
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "rj45Qty");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "rj45Qty");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            labelKey="label"
            options={productsDetail.map((product) => ({
              value: product.usb_qty,
              label: `${product.usb_qty}`,
            }))}
            placeholder="Select Product"
            className="flex-grow-1"
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "usbQty");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "usbQty");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            labelKey="label"
            options={productsDetail.map((product) => ({
              value: product.comport,
              label: `${product.comport}`,
            }))}
            placeholder="Select Product"
            className="flex-grow-1"
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "comport");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "comport");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            labelKey="label"
            options={productsDetail.map((product) => ({
              value: product.hdmi_vga_qty,
              label: `${product.hdmi_vga_qty}`,
            }))}
            placeholder="Select Product"
            className="flex-grow-1"
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "hdmiVgaQty");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "hdmiVgaQty");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "wifi");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "wifi");
            }}
          />
        </Form.Group>
        <Form.Group as={Col} md="12" className="d-flex align-items-center">
          ...
          <Typeahead
            ...
            onChange={(selected) => {
              if (selected && selected.length > 0) {
                handleRowChange(selected[0].value, rowIndex, "detail");
              }
            }}
            onInputChange={(inputText) => {
              // Use inputText for any manually typed value
              handleRowChange(inputText, rowIndex, "detail");
            }}
          />
        </Form.Group>
      </td>
      <td>
        <Form.Group as={Col} md="12">
          ...
          <Form.Control
            type="number"
            onChange={async (e) =>
              await handleRowChange(e.target.value, rowIndex, "price")
            }
          />
        </Form.Group>
      </td>
  ...
  return (
    <>
      <div className="m-5">
        <h1 className="mb-3">
          {!revise ? "Create Quotation IPC" : "Revise Quotation"}
        </h1>

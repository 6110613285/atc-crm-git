...
// ฟังก์ชันใหม่: เพิ่ม PC Log ใหม่
    ...
    $partNum = $_GET['partNum'] ?? '';
    if (!$partNum) {
        echo json_encode(["status" => "error", "message" => "ไม่มีรหัส part_num"]);
        exit;
    }
    ...
    if ($row && $row['picture']) {
        ...
        foreach ($paths as $path) {
            $images[] = "http://" . $_SERVER['HTTP_HOST'] . "/ERP/atc-crm-api" . str_replace('\\', '/', $path);
        }
    }

    echo json_encode(["status" => "success", "images" => $images], JSON_UNESCAPED_SLASHES);
    mysqli_close($link);

}// Function สำหรับลบรูปภาพ
function deletePartImage($link, $partNum, $imageToDelete) {
    // Validate input
    if (empty($partNum) || empty($imageToDelete)) {
        return [
            "status" => "error", 
            "message" => "ข้อมูลไม่ครบ: ต้องมี partNum และ image"
        ];
    }
    
    // Sanitize inputs
    $partNum = mysqli_real_escape_string($link, $partNum);
    $imageToDelete = mysqli_real_escape_string($link, $imageToDelete);
    
    // Get current pictures from database
    $result = mysqli_query($link, "SELECT picture FROM tb_part_no WHERE part_num = '$partNum'");
    ...
    if (!$row) {
        return [
            "status" => "error", 
            "message" => "ไม่พบข้อมูล part_num นี้"
        ];
    }
    
    $existingPictures = $row['picture'] ?? '';
    
    if (!$existingPictures) {
        return [
            "status" => "error", 
            "message" => "ไม่มีรูปภาพใน part_num นี้"
        ];
    }
    
    // Convert to array and check if image exists
    $picturesArray = array_filter(explode(',', $existingPictures));
    $originalCount = count($picturesArray);
    
    // Find and remove the image
    $imageFound = false;
    $filteredArray = [];
    
    foreach ($picturesArray as $img) {
        $cleanImg = trim($img);
        $cleanImageToDelete = trim($imageToDelete);
        
        // Check different path formats
        if ($cleanImg === $cleanImageToDelete || 
            $cleanImg === "./$cleanImageToDelete" || 
            $cleanImg === "uploads/$cleanImageToDelete" || 
            $cleanImg === "./uploads/$cleanImageToDelete" ||
            basename($cleanImg) === basename($cleanImageToDelete)) {
            $imageFound = true;
            // Skip this image (don't add to filtered array)
        } else {
            $filteredArray[] = $cleanImg;
        }
    }
    ...
    if (!$imageFound) {
        return [
            ...
            "existing_images" => $picturesArray,
            "debug_info" => [
                "original_path" => $imageToDelete,
                "cleaned_path" => trim($imageToDelete),
                "basename" => basename($imageToDelete)
            ]
        ];
    }
    
    $newCount = count($filteredArray);
    $updatedList = implode(',', $filteredArray);
    ...
    if (!$stmt) {
        return [
            "status" => "error", 
            "message" => "Database preparation failed: " . $link->error
        ];
    }
    
    $stmt->bind_param("ss", $updatedList, $partNum);
    
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            // Try to delete the physical file
            $possiblePaths = [
                __DIR__ . "/$imageToDelete",
                __DIR__ . "/uploads/$imageToDelete",
                __DIR__ . "./$imageToDelete",
                __DIR__ . "./uploads/$imageToDelete"
            ];
            ...
            return [
                ...
                "remaining_pictures" => $updatedList,
            ...
            return [
                "status" => "error", 
                "message" => "ไม่สามารถอัปเดตฐานข้อมูลได้"
            ];
        }
    } else {
        return [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === "deletePartImage") {
    $partNum = $_POST['partNum'] ?? '';
    $imageToDelete = $_POST['image'] ?? '';
    
    $result = deletePartImage($link, $partNum, $imageToDelete);
